...
class Server {
    ...
    createSeanceClient(transportClient) {
        const seanceClient = {
            emitControllerData: new rxjs_1.Subject(),
            emitNewPage: new rxjs_1.Subject(),
            onControllerAction: transportClient.inputMessage$
                .pipe(operators_1.filter((message) => message.type === "controller-action"), operators_1.map((message) => message.body)),
        ...
        seanceClient.emitControllerData.subscribe((body) => transportClient.outputMessage$.next({
    ...
    createSeance(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const seance = yield this.config.SeancesFactory.createSeance(params);
            const now = new Date();
